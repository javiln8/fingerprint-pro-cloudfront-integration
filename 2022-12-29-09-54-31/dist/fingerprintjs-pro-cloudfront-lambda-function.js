...
function findRule(domain) {
    const punyDomain = Punycode__namespace.toASCII(domain);
    let foundRule = null;
    let foundRulePunySuffix = null;
    for (const rule of domainSuffixList) {
        const suffix = rule.replace(/^(\*\.|!)/, '');
        const rulePunySuffix = Punycode__namespace.toASCII(suffix);
...
function validate(input) {
    const ascii = Punycode__namespace.toASCII(input);
...
function parsePunycode(domain, parsed) {
    ...
    if (parsed.domain) {
        parsed.domain = Punycode__namespace.toASCII(parsed.domain);
    }
    if (parsed.subdomain) {
        parsed.subdomain = Punycode__namespace.toASCII(parsed.subdomain);
    }
    return parsed;
}
